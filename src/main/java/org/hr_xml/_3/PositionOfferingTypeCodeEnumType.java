//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.4-2 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2014.01.06 at 10:26:45 PM EST 
//


package org.hr_xml._3;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for PositionOfferingTypeCodeEnumType.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="PositionOfferingTypeCodeEnumType">
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}normalizedString">
 *     &lt;enumeration value="DirectHire"/>
 *     &lt;enumeration value="Temporary"/>
 *     &lt;enumeration value="TemporaryToHire"/>
 *     &lt;enumeration value="ContractToHire"/>
 *     &lt;enumeration value="Contract"/>
 *     &lt;enumeration value="Internship"/>
 *     &lt;enumeration value="Apprenticeship"/>
 *     &lt;enumeration value="Seasonal"/>
 *     &lt;enumeration value="OnCall"/>
 *   &lt;/restriction>
 * &lt;/simpleType>
 * </pre>
 * 
 */
@XmlType(name = "PositionOfferingTypeCodeEnumType")
@XmlEnum
public enum PositionOfferingTypeCodeEnumType {

    @XmlEnumValue("DirectHire")
    DIRECT_HIRE("DirectHire"),
    @XmlEnumValue("Temporary")
    TEMPORARY("Temporary"),
    @XmlEnumValue("TemporaryToHire")
    TEMPORARY_TO_HIRE("TemporaryToHire"),
    @XmlEnumValue("ContractToHire")
    CONTRACT_TO_HIRE("ContractToHire"),
    @XmlEnumValue("Contract")
    CONTRACT("Contract"),
    @XmlEnumValue("Internship")
    INTERNSHIP("Internship"),
    @XmlEnumValue("Apprenticeship")
    APPRENTICESHIP("Apprenticeship"),
    @XmlEnumValue("Seasonal")
    SEASONAL("Seasonal"),

    /**
     * Would empass work such as substitute and replacement workers.
     * 
     */
    @XmlEnumValue("OnCall")
    ON_CALL("OnCall");
    private final String value;

    PositionOfferingTypeCodeEnumType(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static PositionOfferingTypeCodeEnumType fromValue(String v) {
        for (PositionOfferingTypeCodeEnumType c: PositionOfferingTypeCodeEnumType.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
